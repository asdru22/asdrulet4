...
execute store result score @s block.m_ttc.timer run data get storage ttc:mdata root.temp.block_broken.break_time
data modify entity @s HandItems[0] set from storage ttc:mdata root.temp.equipment.mainhand
scoreboard players set @s block.c_ttc.timer 0
execute store result entity @s ArmorItems[3].tag.base.mining_speed int 1 run scoreboard players get s.mining_speed ttc.d
data modify entity @s ArmorItems[3].tag.base.id set from storage ttc:mdata root.temp.block_broken
