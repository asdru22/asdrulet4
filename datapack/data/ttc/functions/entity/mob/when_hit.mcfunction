...
scoreboard players operation real.damage_recived ttc.d += @s ttc.mob_health
execute if score magic.triggered ttc.d matches 1 run scoreboard players set real.damage_recived ttc.d 100
#tellraw @p {"score":{"name": "real.damage_recived","objective": "ttc.d"}}
...
execute as @a[scores={use_coas=1..}] run function ttc:player/stats/get/active/get
...
scoreboard players operation @s stat.health -= s.damage ttc.d
function ttc:entity/mob/update_hp_display
execute if score @s stat.health matches ..0 run function ttc:entity/mob/on_death